package com.codyy.slr.servet.base;
...
import com.codyy.slr.servet.MyProgressListener;
...
public abstract class BaseServlet extends HttpServlet {
	...
	protected BaseServlet(int uploadMaxSize, List<String> uploadTypeList) {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 6.设置文件最大值
		upload.setFileSizeMax(uploadMaxSize * 1024 * 1024L);
		// 7.设置文件编码
		...
		try {
			...
			while (formItem.hasNext()) {
				...
				if (item.isFormField()) {
					...
					if (!uploadTypeList.contains(suffix)) {
						one.setCode(Constants.FAILED);
	...
	public void setUploadTypeList(List<String> uploadTypeList) {
		this.uploadTypeList = uploadTypeList;
	}
